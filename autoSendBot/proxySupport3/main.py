...
MIN_BREAK = 300   # 5 minutes in seconds
MAX_BREAK = 600  # 10 minutes in seconds
...
async def main():
    ...
    while True:
        print(f"\n[START] Starting a new round for all {len(bot_dirs)} bots...")
        ...
        for bot_dir in bot_dirs:
            ...
            print(f"[INFO] Waiting {break_gap}s before moving to the next bot account.")
            await asyncio.sleep(break_gap)

        # --- NEW LOGIC START ---
        round_delay = 30 * 60  # 30 minutes in seconds
        print(f"\n[ROUND COMPLETE] Finished all bots. Sleeping for 30 minutes before starting again...")
