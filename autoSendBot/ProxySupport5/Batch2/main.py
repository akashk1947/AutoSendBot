...
def get_bot_dirs(root_dir):
    """
    Fixed version of the directory fetcher.
    Uses a tuple-based sort key to prevent TypeError between ints and strings.
    """
    ...
    def sort_key(path):
        ...
        if match:
            # Category 0: Folder has a number (e.g., 'bot1'). Sort by the number.
            return (0, int(match.group()))
        else:
            # Category 1: Folder has no number (e.g., 'bot_backup'). Sort by name.
